...
for wfmIndx = fliplr(wfmIndx_array(1: end))
    ...
    if ismember(wfmIndx, wfmIndx_array)
        ...
        if S.plot_all
            ...
            subplot(2,1,1)
            plot(D.time_full*1e6, D.Pacc_full*1e-3)
            hold on
            ylabel('P_A (kPa)')
            set(gca,'FontSize', 14)
            grid on
            title('driving pressure', 'Fontsize', 12)
            xlim([min(D.time_temp*1e6) max(D.time_temp*1e6)])
            plot(D.time_full*1e6, D.Pacc_full*1e-3)
            subplot(2,1,2)     
            plot(D.time_full*1e6, strain_full(wfmIndx,:))
            hold on
            ylabel('epsilon')
            set(gca,'FontSize', 14)
            grid on
        ...
        % S.range_vec = round(length(input_param.strain_filtered)*0.1);
        ...
        if ~S.calculate_on_server
            ...
            fig_result = figure(201);
            clf
            xlim([-0.2 0.2])
            ylim([-20 100])
            hold on
            plot(strain_vec, surf_tension*1e3, 'linewidth', 2)
            plot(F.strain_input_crop, F.surf_tension_Segers_crop*1e3, '--', 'color', color(2,:), 'linewidth', 3)
            hold off
            grid on
            xlabel('\epsilon')
            ylabel('\sigma (mN/m)')
            title({['measurement index: ' num2str(wfmIndx)]...,
                ['delay = ' num2str(error_min_delay*1e6, '%.2f') ' µs'...,
                ', R_0 = ' num2str(error_min_R0*1e6, '%.2f') ' µm'...,
                ', \sigma(R_0) = ' num2str(error_min_sig_0*1e3, '%.2f') ' mN/m']},...    % , ', P_a = ' num2str(error_min_Pa*1e-3, '%.2f') ' kPa']}, 'fontsize', 14)
                'fontsize', 14)
            legend({'data single bubble', ['similar population' newline '(Segers)']},...
                'location', 'northwest')
            set(gca, 'fontsize', 14)
            ...
            if S.saving_figs
                ...
                saving_folder = [cd '/error minimization figures/'];
                ...
                saveas(fig_result, [saving_folder '/bubble_'...
                    num2str(wfmIndx) '_bestfit'], 'png')
                saveas(fig_result, [saving_folder '/bubble_'...
                    num2str(wfmIndx) '_bestfit'], 'fig')
            end
        end

        %% plot the parameters for all the attempts
        if ~S.calculate_on_server

            fig_parameters_iterations = figure(101);
            clf
            set(gca, 'fontsize', 12)
            set(gcf, 'position', [70 140 1300 700])
            sgtitle(['measurement index: ' num2str(wfmIndx)])
            subplot(2,4,1)
            title('radius')
            hold on
            plot(xestimated(:,1)*1e6, '.', 'markersize', 16)
            plot(min_resnorm_indices, xestimated(min_resnorm_indices,1)*1e6, 'g.', 'markersize', 16)
            plot(min_resnorm_idx, xestimated(min_resnorm_idx,1)*1e6, 'r.', 'markersize', 20)
            yline(error_min_R0*1e6, 'm-', 'linewidth', 1.5)
            xlabel('attempt')
            ylabel('radius (µm)')
            ylim([S.lb(1) S.ub(1)]/input_param.weights(1)*1e6)
            yline(R0est(wfmIndx)*1e6, 'r--')
            grid on

            subplot(2,4,2)
            title('init. surface tension')
            hold on
            plot(xestimated(:,2)*1e3, '.', 'markersize', 16)
            plot(min_resnorm_indices, xestimated(min_resnorm_indices,2)*1e3, 'g.', 'markersize', 16)
            plot(min_resnorm_idx, xestimated(min_resnorm_idx,2)*1e3, 'r.', 'markersize', 20)
            yline(error_min_sig_0*1e3, 'm-', 'linewidth', 1.5)
            xlabel('attempt')
            ylabel('surf. tens. (mN/m)')
            ylim([S.lb(2) S.ub(2)]/input_param.weights(2)*1e3)
            grid on

            subplot(2,4,3)
            title('delay (position)')
            hold on
            plot(xestimated(:,3)*1e6, '.', 'markersize', 16)
            plot(min_resnorm_indices, xestimated(min_resnorm_indices,3)*1e6, 'g.', 'markersize', 16)
            plot(min_resnorm_idx, xestimated(min_resnorm_idx,3)*1e6, 'r.', 'markersize', 20)
            yline(error_min_delay*1e6, 'm-', 'linewidth', 1.5)
            xlabel('attempt')
            ylabel('delay (µs)')
            ylim([S.lb(3) S.ub(3)]/input_param.weights(3)*1e6)%-P.T]*1e6)
            grid on
            ...
            if S.ub(4) - S.lb(4) > 1
                ...
                hold on
                plot(xestimated(:,4), '.', 'markersize', 16)
                plot(min_resnorm_indices, error_min_Pa_check*1e-3, 'g.', 'markersize', 16)
                % plot(min_resnorm_idx, P.Pa*1e-3, 'r.', 'markersize', 20)
                plot(min_resnorm_idx, xestimated(min_resnorm_idx,4), 'r.', 'markersize', 20)
                % plot(min_RMSE_indices, vestimated(min_RMSE_indices,4), 'm.', 'markersize', 20)
                yline(error_min_Pa*1e-3, 'm-', 'linewidth', 1.5)
                xlabel('attempt')
                ylabel('pressure (kPa)')
                ylim([S.lb(4) S.ub(4)]/input_param.weights(4)*1e-3)
                grid on
            end
            ...
            title('resnorm')
            hold on
            plot(resnorm, '.', 'markersize', 16)
            plot(min_resnorm_indices, min_resnorms, 'g.', 'markersize', 16)
            plot(min_resnorm_idx, min_resnorm, 'r.', 'markersize', 20)
            xlabel('attempt')
            grid on
            if S.attempts>1
                ylim([min_resnorm max(resnorm(xestimated(:,1) > 1.1e-6))])
            end
            ...
            grid on
            if S.attempts>1
                % ylim([min_resnorm max(resnorm(vestimated(:,1) > 1.1e-6))])
                ylim([0 2*median(resnorm)])
            end

            subplot(2,4,7)
            title('exitflag')
            plot(exitflag', '.', 'markersize', 16)
            hold on
            plot(min_resnorm_indices, exitflag(min_resnorm_indices)', 'g.', 'markersize', 16)
            plot(min_resnorm_idx, exitflag(min_resnorm_idx)', 'r.', 'markersize', 20)
            xlabel('attempt')
            grid on

            subplot(2,4,8)
            title('iterations')
            plot([output.iterations]', '.', 'markersize', 16)
            hold on
            plot(min_resnorm_indices, [output(min_resnorm_indices).iterations]', 'g.', 'markersize', 16)
            plot(min_resnorm_idx, [output(min_resnorm_idx).iterations]', 'r.', 'markersize', 20)
            xlabel('attempt')
            grid on
            ...
            if S.saving_figs
                ...
                saveas(fig_parameters_iterations, [saving_folder '/bubble_'...
                    num2str(wfmIndx) '_fitting_parameters'], 'png')
                saveas(fig_parameters_iterations, [saving_folder '/bubble_'...
                    num2str(wfmIndx) '_fitting_parameters'], 'fig')
            end
        end
        ...
        if S.save_data
            ...
            saving_dir = [saving_folder '/bubble ' num2str(wfmIndx)];
            saving_file_name = [saving_dir '.mat'];

            save(saving_file_name, 'P', 'S', 'F', 'D', ...
                'input_param', 'strain_vec', 'strain_vec_crop', 'strain_grid',...
                'dstrain_vec', 'dstrain_grid',...
                'surf_tension', 'surf_tension_crop', 'surf_tension_old', 'surf_tension_smooth',...
                'error_min_delay', 'error_min_R0', 'error_min_sig_0', 'error_min_Pa',...
                'viscoelastic_contrib_int', 'viscoelastic_contrib_int_old', 'elastic_contrib', 'elastic_contrib_old',...
                'min_resnorms', 'min_resnorm_indices', 'initial_guess', ... 
                'min_residual', 'min_residual_idx', 'min_resnorm', 'min_resnorm_idx', ...
                'exitflag', 'output', 'opts',...
                'residuals', 'resnorm', 'xestimated')                
